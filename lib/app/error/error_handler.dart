...
import '../../shared/shared_responses/error_response.dart';
import 'failure.dart';
  ...
  Failure _getFailureFromResponse({Response<dynamic>? response}) {
    try {
      if (response != null && response.statusCode != null) {
        if ((response.statusCode == 406 || response.statusCode == 422 || response.statusCode == 400 || response.statusCode == 401) && response.data.toString().contains('errors')) {
          ...
          if (messages.isNotEmpty) {
            return Failure(code: response.statusCode ?? ResponseCode.DEFAULT, message: messages.join(', '));
          } else if (errorResponse.message != null && errorResponse.message!.isNotEmpty) {
            return Failure(code: response.statusCode ?? ResponseCode.DEFAULT, message: errorResponse.message ?? ResponseMessage.DEFAULT);
          } else {
            return DataSource.DEFAULT.getFailure();
          }
        } else if (response.statusCode == 406 || response.statusCode == 422) {
          return Failure(code: response.statusCode ?? ResponseCode.DEFAULT, message: response.data?['message'] ?? ResponseMessage.DEFAULT);
        } else {
          return Failure(code: response.statusCode ?? ResponseCode.DEFAULT, message: response.statusMessage ?? ResponseMessage.DEFAULT);
        }
      } else {
        return DataSource.DEFAULT.getFailure();
      }
  ...
  Failure getFailure() {
    switch (this) {
      ...
      case DataSource.INTERNAL_SERVER_ERROR:
        return Failure(code: ResponseCode.INTERNAL_SERVER_ERROR, message: ResponseMessage.INTERNAL_SERVER_ERROR);
...
class ResponseMessage {
  ...
  static const String FORBIDDEN = 'Forbidden Request, Try again later'; //failure api user not auth
  static const String UNAUTHORIZED = 'User is unauthorized, Try again later'; //failure api rejected request
  static const String INTERNAL_SERVER_ERROR = 'Something went wrong, Try again later '; //crash in server side
  static const String NOT_FOUND = 'Not found, Try again later '; // api not found in rote of server
//local status code
