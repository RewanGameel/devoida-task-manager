...
import '../widget/navigation_component.dart';
...
class StateRenderer extends StatelessWidget {
  ...
  StateRenderer({Key? key, required this.retryActionFunction, this.message = AppStrings.loading, this.title = '', required this.stateRendererType}) : super(key: key);
  ...
  Widget _getStateContent(BuildContext context) {
    switch (stateRendererType) {
      ...
      case StateRendererType.fullScreenErrorState:
        return _getItemsColumn([_getAnimatedImage(Assets.assetsJsonError), _getMessage(message), _getRetryButton(AppStrings.reTryAgain, context)]);
  ...
  Widget _getErrorPopupDialog(BuildContext context, List<Widget> children) {
    return Dialog(
      ...
      child: Container(
        margin: getMargin(horizontal: AppPadding.p16, vertical: AppPadding.p16, isContent: false),
        decoration: BoxDecoration(
          color: ColorManager.white,
          shape: BoxShape.rectangle,
          borderRadius: BorderRadius.circular(AppSize.s12),
